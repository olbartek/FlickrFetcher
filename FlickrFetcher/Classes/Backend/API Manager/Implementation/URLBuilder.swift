...
final class URLBuilder: URLBuilderType {
    ...
    func photosSearchUrl(withTag tag: String) -> URL {
        let urlString = Flickr.APIURL + "?method=\(Flickr.Method.PhotosSearch)&api_key=\(Flickr.APIKey)&tags=\(tag)&format=json&nojsoncallback=1"
        return URL(string: urlString)!
    }
    
    func photosGetRecentUrl(withTag tag: String) -> URL {
        let urlString = Flickr.APIURL + "?method=\(Flickr.Method.PhotosGetRecent)&api_key=\(Flickr.APIKey)&tags=\(tag)&format=json&nojsoncallback=1"
        return URL(string: urlString)!
    }
